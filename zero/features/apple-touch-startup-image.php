...
function zero_apple_touch_startup_image_wp_head( ) {
  ...
  // try the cache first
  $image = wp_cache_get( 'apple-touch-startup-image', 'ZEROTHEME' );
  ...
  if($icons == false || (defined('WP_DEBUG') && WP_DEBUG)) {
    ...
    // Save to cache
    wp_cache_set( 'apple-touch-startup-image', $image, 'ZEROTHEME' );
  }
  
  // output the startup image
  if($image == 'yes') {
    echo '<meta name="apple-mobile-web-app-capable" content="yes" />';
  	echo '<link rel="apple-touch-startup-image" href="'.get_bloginfo( 'stylesheet_directory' ).'/img/apple-touch-startup-image.png" />';
  }
